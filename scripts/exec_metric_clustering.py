...
def get_fcluster_result(df, metric, method, cluster, metric_cluster):
    ...
    plot_file_name = '%s%s/%s/%s/%s/%s/cluster.png' %(path_indexed_metric_src, problem_id, metric, method, str(cluster), str(metric_cluster))
    plt.savefig(plot_file_name, dpi = 1000)
    plt.clf()
    
    return fcluster(result, num_metric_clusters, criterion='maxclust')



def exec_copy(src_list, label, metric, method, num_cluster, num_metric_cluster, problem_id):
    index = 0
    for src_name in src_list:
        path_src_file = '%s%s' % (path_src, src_name)
        if os.path.exists(path_src_file):
            path_copy_src_file = '%s/%s/%s/%s/%s/%s' % (path_indexed_metric_src, problem_id, metric, method, label[index], src_name)
...
def exec_all_metric_clustering(problem_id):
    ...
    for num_cluster in clusters:
        for num_metric_cluster in metric_clusters:
                    ...
                    if not os.path.exists(path_cluster):
                        ...
                        with open(path_metric_csv, "r", encoding="utf-8") as csv_file:
                            ...
                            for csv_row in header:
                                tmp_file_name = '%s%s%s' % (csv_row[0], csv_row[1], csv_row[2])
                                # ディレクトリに注目している行のファイルが存在すれば
                                if tmp_file_name in src_list:
                                    metric_list = [csv_row[3], csv_row[4], csv_row[5], csv_row[6], csv_row[7], csv_row[8], csv_row[9], csv_row[12], csv_row[14], csv_row[15], csv_row[16], csv_row[17]]
                        ...
                        label = get_fcluster_result(data_frame, metric, method, num_cluster, num_metric_cluster)
                        exec_copy(src_list, label, metric, method, num_cluster, num_metric_cluster, problem_id)
